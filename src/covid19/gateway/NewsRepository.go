...
func (dbRepo *DatabaseRepository) AddNews(news *domain.News) (*database.News, error) {
	...
	u.Copy(&DBNews, news)
	if len(news.Date) > 0 {
		DBNews.CreatedAt, _ = time.Parse("2006-01-02", news.Date)
	}
...
func (dbRepo *DatabaseRepository) GetNews(createDate string, endDate string) ([]database.News, error) {
	var DBNews []database.News
	err := dbRepo.GetDBHandle().Where("date(created_at) >= ? and date(created_at) <= ?", createDate, endDate).Find(&DBNews).Error
	return DBNews, err
}
